...
bool DecodeobjModel(const char * fileConetext,unsigned short ** indices,
                    int &indiceCount, Vertex **vertices, int &vertexCount){
    if(fileConetext == nullptr){
        printf("decode obj model fail ,file is nill");
        return false;
    }
    ...
    std:: stringstream ssFile(fileConetext);
    char szOneline[128];
    std:: string temp;
