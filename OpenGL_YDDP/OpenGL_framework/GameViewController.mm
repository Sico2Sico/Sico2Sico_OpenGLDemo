...
//    colorAtteriLocation = glGetAttribLocation(gpuProgram,"pos");
//    posLocation = glGetAttribLocation(gpuProgram,"color");
//
...
//    textureLocation = glGetUniformLocation(gpuProgram,"U_MainTexture");
    ...
    vbo = CreateBufferObject(GL_ARRAY_BUFFER,
                             sizeof(Particel)*nParticleConut,
                             particles,
                             GL_STATIC_DRAW);
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glDrawArrays(GL_POINTS, 0, nParticleConut);
